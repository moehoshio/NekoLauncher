...
TEST_F(LangTest, LoadTranslations) {
    ...
    EXPECT_EQ(translations["testKey"], "Test Value");
}
...
TEST_F(LangTest, LoadTranslationsCaching) {
    // First load
    auto trans1 = loadTranslations("en", testLangDir);
    EXPECT_FALSE(trans1.empty());
    
    // Second load of same language (should use cache)
    auto trans2 = loadTranslations("en", testLangDir);
    EXPECT_EQ(trans1, trans2);
    
    // Load different language (should clear cache)
    auto trans3 = loadTranslations("zh_tw", testLangDir);
...
TEST_F(LangTest, TranslationLookup) {
    ...
    std::string result = tr("testKey", "Not found", translations);
    EXPECT_EQ(result, "Test Value");
    
    result = tr("greeting", "Not found", translations);
...
TEST_F(LangTest, TranslationFallback) {
    ...
    std::string result = tr("nonExistentKey", "Fallback message", translations);
...
TEST_F(LangTest, TranslationEmptyFile) {
    ...
    std::string result = tr("anyKey", "Default", emptyJson);
...
// Test withReplaced lambda
TEST_F(LangTest, WithReplacedLambda) {
    std::string input = "Hello {name}!";
    std::map<std::string, std::string> replacements = {
        {"{name}", "Charlie"}
    };
    
    std::string result = withReplaced(input, replacements);
...
TEST_F(LangTest, LanguageKeys) {
    ...
    EXPECT_STREQ(keys::action::object.data(), "Action");
    EXPECT_STREQ(keys::action::networkRequest.data(), "networkRequest");
    EXPECT_STREQ(keys::action::downloadFile.data(), "downloadFile");
    
    // Test object namespace
    EXPECT_STREQ(keys::object::object.data(), "Object");
    EXPECT_STREQ(keys::object::maintenance.data(), "maintenance");
    EXPECT_STREQ(keys::object::update.data(), "update");
    
    // Test button namespace
    EXPECT_STREQ(keys::button::ok.data(), "ok");
    EXPECT_STREQ(keys::button::cancel.data(), "cancel");
    EXPECT_STREQ(keys::button::retry.data(), "retry");
    
    // Test error namespace
    EXPECT_STREQ(keys::error::unknownError.data(), "unknownError");
    EXPECT_STREQ(keys::error::networkError.data(), "networkError");
    
    // Test update namespace
    EXPECT_STREQ(keys::update::checkingForUpdates.data(), "checkingForUpdates");
    EXPECT_STREQ(keys::update::updateAvailable.data(), "updateAvailable");
}
...
TEST_F(LangTest, CompleteTranslationWorkflow) {
    ...
    // Get translation
    std::string greeting = tr("greeting", "Not found", translations);
    EXPECT_EQ(greeting, "Hello ZH");
    
    // Use placeholders
    std::string placeholderText = tr("placeholder", "Not found", translations);
