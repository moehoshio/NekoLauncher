...
namespace neko::core::thread {
    ...
    struct WorkerInfo {
        ...
        WorkerInfo(std::thread &&t, neko::uint64 i)
            : thread(t), id(i), startTime(std::chrono::steady_clock::now()) {}
                ...
                { // Check if this worker is marked for exit
                    ...
                    if (exitWorkerIds.count(workerId)) {
                        ...
                        workers.erase(std::remove_if(workers.begin(), workers.end(),
                                                     [workerId](const WorkerInfo &w) { return w.id == workerId; }),
                                      workers.end());
                        break;
                    }
                }
    ...
    public:
        ...
        auto submitToWorker(neko::uint64 workerId, auto &&function, auto &&...args)
            ...
            std::lock_guard<std::mutex> lock(workerMutex);
            auto it = std::find_if(workers.begin(), workers.end(), [workerId](const WorkerInfo &w) { return w.id == workerId; });
            if (it == workers.end())
                throw ex::OutOfRange("Worker not found");
        ...
        void setThreadCount(neko::uint64 newThreadCount) {
            ...
            targetWorkerCount.store(newThreadCount);
